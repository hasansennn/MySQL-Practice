...
select sirket
from sirketler
where personel_sayisi>15000; -- bize ford ve toyota dondurur
...
from personel
where sirket in 
(select sirket from sirketler where personel_sayisi>15000); -- Bu sekilde komutumuzu dinamiklestirdik
...
select sirket, personel_sayisi,(select max(maas)from personel where sirketler.sirket=personel.sirket )
 max_maas,(select min(maas) from personel where sirketler.sirket=personel.sirket)  min_maas
from sirketler;
...
select sirket, personel_sayisi, (select sum(maas)from personel where sirketler.sirket=personel.sirket)top_maas 
from sirketler;
